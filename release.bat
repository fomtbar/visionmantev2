...
:: ─────────────────────────────────────────────────────────────────
:: PASO 2 — Verificar PyInstaller (sin tocar dependencias)
:: ─────────────────────────────────────────────────────────────────
echo.
echo  [2/5] Verificando PyInstaller en env1...
pyinstaller --version >nul 2>&1
if errorlevel 1 (
    echo  [ERROR] PyInstaller no esta instalado en env1.
    echo          Instalar manualmente: env1\Scripts\pip install pyinstaller
    pause & exit /b 1
)
for /f "tokens=*" %%v in ('pyinstaller --version 2^>nul') do set PI_VER=%%v
